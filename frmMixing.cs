...
namespace ScaleApp
{
    public partial class frmMixing : Form
    {
        ...
        //<-- declares a SerialPort Variable to be used throughout the form
        ...
        private void CreateMixRaw()
        {
            try
            {
                string strTedColorCode = gridViewMaterialBom.GetRowCellValue(0, "materialcode").ToString();
                string strTedColorName = gridViewMaterialBom.GetRowCellValue(0, "materialname").ToString();
                ...
                cmd.Parameters.AddWithValue("@materialCode", strTedColorCode);
                cmd.Parameters.AddWithValue("@materialName", strTedColorName);
        ...
        private void LoadLueOperator()
        {
            ...
            try
            {
                using (SqlDataAdapter SqlDa = new SqlDataAdapter("sp_getOperatorsMix", conn))
                {
                    SqlDa.SelectCommand.CommandType = CommandType.StoredProcedure;
                    SqlDa.Fill(ds);
                }
        ...
        private void LoadGridControl1()
        {
            ...
            try
            {
                ...
                SqlDataAdapter SqlDaMixRaw = new SqlDataAdapter("sp_getFullMixRaws4", conn);
                ...
                //Orde column of Detail Grid
                gridView3.Columns["CrushRawId"].VisibleIndex = 0;
                gridView3.Columns["CreateTime"].VisibleIndex = 1;
                gridView3.Columns["ShiftName"].VisibleIndex = 2;
                gridView3.Columns["RecycledID"].VisibleIndex = 3;
                gridView3.Columns["MixBacode"].VisibleIndex = 4;
                gridView3.Columns["OperatorName"].VisibleIndex = 5;
                gridView3.Columns["ColorCode"].VisibleIndex = 6;
                gridView3.Columns["WeightRecycle"].VisibleIndex = 7;
                gridView3.Columns["LossTypeName"].VisibleIndex = 8;                
                gridView3.Columns["MachineName"].VisibleIndex = 9;                
                gridView3.Columns["Posted"].VisibleIndex = 10;

                //Set column's width of detail GridView
                gridView3.Columns["CrushRawId"].Width = 40;
                gridView3.Columns["CreateTime"].Width = 120;
                gridView3.Columns["ShiftName"].Width = 40;
                ...
                gridView3.Columns["CreateTime"].DisplayFormat.FormatType = DevExpress.Utils.FormatType.DateTime;
                gridView3.Columns["CreateTime"].DisplayFormat.FormatString = "MM/dd/yyyy hh:mm:ss";

                gridView3.Columns["WeightRecycle"].DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric;
                gridView3.Columns["WeightRecycle"].DisplayFormat.FormatString = "{0:n3}";
        ...
        private void UpdatePosted()
        {
            ...
            if (i != 0)
            {
                XtraMessageBox.Show("Mix Raw Lot posted !", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                loadGridView1();
                spbPost.Enabled = false;
                spbSave.Enabled = false;
                LoadGridControl1();
        ...
        private void UpdateBomMaterialWeight()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dataRowCount; i++)
                {
                    ...
                    if (view.GetRowCellValue(i, "materialcode").ToString().Contains("RPM") || view.GetRowCellValue(i, "materialcode").ToString().Contains("RCM"))
                    {
                        UpdateMaterialMixRaw(view.GetRowCellValue(i, "materialcode").ToString(), view.GetRowCellValue(i, "materialname").ToString(), qrMixLotID.Text);
                    }
                }
                ScaleApp.Common.DataOperation.disconnect();
            }
        ...
        private void UpdateMaterialMixRaw(string materCode, string materName, string mixQRCode)
        {
            GridView view = gridViewMaterialBom;
            try
            {
                ...
                SqlConnection conn = new SqlConnection(connStr);                
                conn.Open();
                SqlCommand cmd = new SqlCommand("sp_editMixRawMaterial", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                
                cmd.Parameters.AddWithValue("@materialName", materCode);
                cmd.Parameters.AddWithValue("@materialName1", materName);                
                cmd.Parameters.AddWithValue("@mixBacode", mixQRCode);
        ...
        {
            if (lueRecycled.EditValue.IsNull())
            {
                return;
            }
            else
            {
                if (!lueRecycled.GetColumnValue("StepName").IsNullOrEmpty())
                {
                    cmbStep.SelectedValue = lueRecycled.GetColumnValue("StepName").ToString();
                }

                if (!lueRecycled.GetColumnValue("ProductName").IsNullOrEmpty())
                {
                    lueProduct.EditValue = lueRecycled.GetColumnValue("ProductName");
                }
            }
        }
